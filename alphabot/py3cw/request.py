...
class IPy3CW:
    def request(
        self, entity: str, action: str = "", action_id: str = None, payload: any = None
    ):
        pass
...
class Py3CW(IPy3CW):
    def __init__(self, key: str, secret: str, request_options=None):
        ...
        self.request_retry_status_codes = request_options.get(
    ...
    def __make_request(
        self, http_method: str, path: str, params: any, payload: any, retry_count=0
    ):
        """
        ...
        signature = self.__generate_signature(
            relative_url, (json.dumps(payload) if payload is not None else "")
        )
        ...
        try:
            response = self.session.request(
                method=http_method,
                url=absolute_url,
                headers={"APIKEY": self.key, "Signature": signature},
                json=payload,
            ...
            if type(response_json) is dict and "error" in response_json:
                ...
                if (
                    error_status_code
                    and error_status_code in self.request_retry_status_codes
                    and retry_count < self.request_retries_count
                ):
            ...
            return {
                "error": True,
                "msg": "Other error occurred: {} {} {}.".format(
