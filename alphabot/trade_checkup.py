...
def get_tp_sl_reset_due_to_dca(_trade_status, strat_states, strat, description):
    ...
    if current_units == dca_stages[dca_stage]['cumulative_units']:
        ...
        #print(
        #    f"{description} current_units {current_units} matches expected {dca_stages[dca_stage]['cumulative_units']} for "
        #    f"stage {dca_stage}"
        #)
        return None, None, None
    ...
    print(
        f"{description} new TP pct is {new_tp_pct}, SL pct is still {sl_pct}. Starting from new average entry of "
        f"{new_entry}, new TP price is {new_tp_price} and new SL price is {new_sl_price}")
...
def log_profit_and_roe(
    ...
    dca_stage = strat_states[strat]["status"].get("dca_stage")
    dca_stages = strat_states[strat]["status"]["dca_stages"]

    #if not h.is_trade_closed(_trade_status=_trade_status, logger=logger):
    if h.is_trade_open(_trade_status=_trade_status):
    ...
    if h.is_trade_closed(_trade_status):
        ...
        print(
            ...
            f"{current_units}, max units {max_units} and asset allocation {asset_allocation}")
        ...
        if user == "latham":
            h.send_email(
                to="lathamfell@gmail.com", subject="AlphaBot Trade Closed", body=trade_summary
            )
